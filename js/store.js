...
storeSido.addEventListener("change", () => {
  ...
  if (sidoObj && sidoObj.sigunguList) {
    const options = sidoObj.sigunguList.map((sg) => `<option value="${sg.sigungu}">${sg.sigungu}</option>`).join("");
...
function getCurrentData() {
  return currentTab === "direct" ? directStores : partnerStores;
}
...
function applyFilter() {
  ...
  filteredStores = data.filter((d) => {
    ...
    //탭이 파트너일때 라운즈렌즈 체크하면
    const matchRounzLens = currentTab === "partner" ? !rounzLensFilter || d.rounzLens === true : true;
...
function getCurrentStoreList() {
  ...
  return activeContent ? activeContent.querySelector(".store-list") : null;
}
